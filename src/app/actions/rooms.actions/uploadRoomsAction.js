...
export async function uploadRoomsAction(formData) {
  try {
    ...
    // 1️⃣ Assign room_no separately
    const lastRoom = await Room.findOne().sort({ room_no: -1 }).lean();
    let nextRoomNo = lastRoom ? lastRoom.room_no + 1 : 101;
    rooms = rooms.map(r => ({ ...r, room_no: nextRoomNo++ }));

    // 2️⃣ Bulk insert all rooms at once
